...
// Get category-specific content directory
function getCategoryContentDirectory(category: string): string {
...
// Get content slugs for a specific category
export function getCategoryContentSlugs(category: string): string[] {
  try {
    const categoryDir = getCategoryContentDirectory(category)
    ...
    console.warn(`Error reading category directory for ${category}:`, error)
    return []
  }
}
...
// Get content data for a specific category and slug
export async function getCategoryContentData(category: string, slug: string): Promise<ContentData | null> {
  try {
    ...
    if (!fs.existsSync(fullPath)) {
      throw new Error(`Content file not found for category ${category}, slug: ${slug}`)
    }
    ...
    console.error(`Error loading content for category ${category}, slug ${slug}:`, error)
    return null
  }
}
...
export async function getCategoryContent(category: string): Promise<ContentData[]> {
...
// Build search index for a specific category
export async function buildCategorySearchIndex(category: string): Promise<SearchIndexItem[]> {
  const allContent = await getCategoryContent(category)
  const index: SearchIndexItem[] = []
  ...
  allContent.forEach(page => {
    ...
    // Extract and index sections
    const sections = extractSections(page.contentHtml, page.title, page.slug)
    // Update section paths to include category
    const categorySections = sections.map(section => ({
      ...section,
      path: `/${category}/${section.slug}#${section.sectionId}`,
      category: page.category || category
    }))
    index.push(...categorySections)
  })
